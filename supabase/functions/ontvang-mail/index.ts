...
import { leesFactuur, type Lezing } from '../_gedeeld/factuurlezer.ts'
...
async function boekAutomatisch(
   ...
   *
   * Staat Truckwash als afzender op het stuk, dan gaat de kostenpost die deze
   * functie zojuist aanmaakte weer weg. Dat kan hier nog veilig: er heeft nog
  ...
  if (lezing.richting === 'verkoop') {
    const slot = await eigenNummerOpHetStuk(lezing)

    if (slot.bevestigd) {
      console.log(`[ontvang-mail] ${expenseId} is een verkoopfactuur: ${slot.waarom}`)
      if (await zetAlsVerkoopfactuur(berichtId, expenseId, vanNaam, onderwerp)) return
      /*
       ...
       */
      console.error(`[ontvang-mail] ${expenseId}: verkoop niet kunnen merken, wordt gewoon ingevuld`)
    } else {
       ...
       */
      await twijfelErbij(expenseId, lezing,
      ...
      console.log(`[ontvang-mail] ${expenseId}: model zegt verkoop, niet bevestigd (${slot.waarom})`)
      return
    }
  }
...
function kaal(waarde: unknown): string {
  return String(waarde ?? '').toUpperCase().replace(/[^A-Z0-9]/g, '')
}

async function eigenNummerOpHetStuk(
  lezing: Lezing,
  ...
  if (error) {
    console.warn('[ontvang-mail] eigen nummers lezen: ' + error.message)
 ...
 */
async function twijfelErbij(expenseId: string, lezing: Lezing, zin: string) {
  const { error } = await admin
    .from('expenses')
    .update({ gelezen: { ...lezing, twijfel: [...lezing.twijfel, zin] } })
    .eq('id', expenseId)
  if (error) console.warn('[ontvang-mail] twijfel bijschrijven: ' + error.message)
}
 ...
 */
async function zetAlsVerkoopfactuur(
  berichtId: string,
  expenseId: string,
  vanNaam: string,
  onderwerp: string,
  ...
  const { error: merk } = await admin
    ...
    .eq('id', berichtId)
  if (merk) {
    console.error('[ontvang-mail] bericht merken als verkoop: ' + merk.message)
    return false
  }
  ...
  if (weg) {
    ...
    console.error('[ontvang-mail] kostenpost van verkoopfactuur weghalen: ' + weg.message)
  }
   ...
   */
  await meldManagement(berichtId, {
    kind: 'info',
  ...
  console.log(`[ontvang-mail] ${expenseId} was een verkoopfactuur; kostenpost weggehaald`)
  return true
}
...
async function meldManagement(
  ...
  for (const baas of bazen ?? []) {
    ...
    if (error && error.code !== '23505') {
      console.warn('[ontvang-mail] melding aan management: ' + error.message)
    }
  }
}
